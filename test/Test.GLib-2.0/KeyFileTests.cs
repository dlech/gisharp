...
namespace GISharp.Test.GLib
{
    ...
    public class KeyFileTests
    {
        ...
        public void TestString()
        {
            ...
            Assert.That(
                () => keyFile.GetString(TestGroup, TestKey1),
            ...
            Assert.That(
                () => keyFile.GetString(TestGroup, TestKey2),
        ...
        public void TestBoolean()
        {
            using var keyFile = new KeyFile();
            Assert.That(
                () => keyFile.GetBoolean(TestGroup, TestKey1),
                ThrowsGErrorException(KeyFileError.GroupNotFound),
                "Trying to get a non-existant group should throw an exception"
            );
            ...
            Assert.That(
                () => keyFile.GetBoolean(TestGroup, TestKey2),
                ThrowsGErrorException(KeyFileError.KeyNotFound),
                "Trying to get a non-existant key should throw an exception"
            );
        }
        ...
        public void TestInteger()
        {
            using var keyFile = new KeyFile();
            Assert.That(
                () => keyFile.GetInteger(TestGroup, TestKey1),
                ThrowsGErrorException(KeyFileError.GroupNotFound),
                "Trying to get a non-existant group should throw an exception"
            );
            ...
            Assert.That(
                () => keyFile.GetInteger(TestGroup, TestKey2),
                ThrowsGErrorException(KeyFileError.KeyNotFound),
                "Trying to get a non-existant key should throw an exception"
            );
        }
        ...
        public void TestInt64()
        {
            using var keyFile = new KeyFile();
            Assert.That(
                () => keyFile.GetInt64(TestGroup, TestKey1),
                ThrowsGErrorException(KeyFileError.GroupNotFound),
                "Trying to get a non-existant group should throw an exception"
            );
            ...
            Assert.That(
                () => keyFile.GetInt64(TestGroup, TestKey2),
                ThrowsGErrorException(KeyFileError.KeyNotFound),
                "Trying to get a non-existant key should throw an exception"
            );
        }
        ...
        public void TestUInt64()
        {
            using var keyFile = new KeyFile();
            Assert.That(
                () => keyFile.GetUint64(TestGroup, TestKey1),
                ThrowsGErrorException(KeyFileError.GroupNotFound),
                "Trying to get a non-existant group should throw an exception"
            );
            ...
            Assert.That(
                () => keyFile.GetUint64(TestGroup, TestKey2),
                ThrowsGErrorException(KeyFileError.KeyNotFound),
                "Trying to get a non-existant key should throw an exception"
            );
        }
        ...
        public void TestDouble()
        {
            using var keyFile = new KeyFile();
            Assert.That(
                () => keyFile.GetDouble(TestGroup, TestKey1),
                ThrowsGErrorException(KeyFileError.GroupNotFound),
                "Trying to get a non-existant group should throw an exception"
            );
            ...
            Assert.That(
                () => keyFile.GetDouble(TestGroup, TestKey2),
                ThrowsGErrorException(KeyFileError.KeyNotFound),
                "Trying to get a non-existant key should throw an exception"
            );
        }
        ...
        public void TestStringList()
        {
            using var keyFile = new KeyFile();
            Assert.That(
                () => keyFile.GetStringList(TestGroup, TestKey1),
                ThrowsGErrorException(KeyFileError.GroupNotFound),
                "Trying to get a non-existant group should throw an exception"
            );
            ...
            Assert.That(
                () => keyFile.GetStringList(TestGroup, TestKey2),
                ThrowsGErrorException(KeyFileError.KeyNotFound),
                "Trying to get a non-existant key should throw an exception"
            );
        }
        ...
        public void TestBooleanList()
        {
            using var keyFile = new KeyFile();
            Assert.That(
                () => keyFile.GetBooleanList(TestGroup, TestKey1),
                ThrowsGErrorException(KeyFileError.GroupNotFound),
                "Trying to get a non-existant group should throw an exception"
            );
            ...
            Assert.That(
                () => keyFile.GetBooleanList(TestGroup, TestKey2),
                ThrowsGErrorException(KeyFileError.KeyNotFound),
                "Trying to get a non-existant key should throw an exception"
            );
        }
        ...
        public void TestIntegerList()
        {
            using var keyFile = new KeyFile();
            Assert.That(
                () => keyFile.GetIntegerList(TestGroup, TestKey1),
                ThrowsGErrorException(KeyFileError.GroupNotFound),
                "Trying to get a non-existant group should throw an exception"
            );
            ...
            Assert.That(
                () => keyFile.GetIntegerList(TestGroup, TestKey2),
                ThrowsGErrorException(KeyFileError.KeyNotFound),
                "Trying to get a non-existant key should throw an exception"
            );
        }
        ...
        public void TestDoubleList()
        {
            using var keyFile = new KeyFile();
            Assert.That(
                () => keyFile.GetDoubleList(TestGroup, TestKey1),
                ThrowsGErrorException(KeyFileError.GroupNotFound),
                "Trying to get a non-existant group should throw an exception"
            );
            ...
            Assert.That(
                () => keyFile.GetDoubleList(TestGroup, TestKey2),
                ThrowsGErrorException(KeyFileError.KeyNotFound),
                "Trying to get a non-existant key should throw an exception"
            );
        }
