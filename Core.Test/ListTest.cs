...
namespace Core.Test
{
    ...
    public class ListTest
    {
        ...
        public void TestInsertSorted ()
        {
            ...
            Assume.That (list.Length, Is.EqualTo (0));
            // adding new item takes ownership of old pointer
            var newList = list.InsertSorted (new TestWrappedNative (1), compareFunc);
            Assert.That (list.Length, Is.EqualTo (0));
            Assert.That (list.Owned, Is.False);
            // return value is now owned head of list
            Assert.That (newList.Length, Is.EqualTo (1));
            ...
            newList = newList.InsertSorted (new TestWrappedNative (3), compareFunc);
            newList = newList.InsertSorted (new TestWrappedNative (2), compareFunc);
            ...
            Assert.That (() => newList.InsertSorted (new TestWrappedNative (4), null),
        ...
        {
            var list = new List<TestWrappedNative> ().Append (null);
        ...
        public void TestRemoveLink ()
        {
            ...
            list = new List<TestWrappedNative> ().Append (null);
            // calling remove on unowned pointer is not allowed
        ...
        {
            var list = new List<TestWrappedNative> ()
                .Append (new TestWrappedNative (1))
                .Append (new TestWrappedNative (2));
        ...
        public void TestCopyDeep ()
        {
            ...
            CopyFuncCallback<TestWrappedNative> copyFunc = (src) => {
                ...
                var ret = new TestWrappedNative (src.Value);
                return ret;
            };
            var list = new List<TestWrappedNative> ().Append (null);
            Assume.That (list.Length, Is.EqualTo (1));
        ...
        {
            var list = new List<TestWrappedNative> ()
                .Prepend (new TestWrappedNative (1))
                .Prepend (new TestWrappedNative (2));
            Assume.That (list.Length, Is.EqualTo (2));
        ...
        public void TestConcat ()
        {
            ...
            Assert.That (() => List<TestWrappedNative>.Concat (list1, list2),
                Throws.InvalidOperationException);
            // make sure null values are allowed
            newList = List<TestWrappedNative>.Concat (null, null);
            Assert.That (newList.Length, Is.EqualTo (0));
            Assert.That (newList.Owned, Is.True);
        }
        ...
        public void TestForeach ()
        {
            ...
            var list = new List<TestWrappedNative> ()
                .Append (new TestWrappedNative (1))
                .Append (new TestWrappedNative (2))
                .Append (new TestWrappedNative (3));
            ...
            Assert.That (() => list.Foreach (null),
                Throws.InstanceOf<ArgumentNullException> ());
            // calling on unowned pointer is not allowed
            Assert.That (() => list.First.Foreach (foreachFunc),
                Throws.InvalidOperationException);
        }
        ...
        public void TestPrevious ()
        {
            var list = new List<TestWrappedNative> ()
                ...
                .Append (new TestWrappedNative (2))
                .Append (new TestWrappedNative (3))
                .Append (new TestWrappedNative (4));
            ...
            previous = list.Last.Previous;
            Assert.That (previous.Data.Value, Is.EqualTo (3));
        ...
        public void TestNthData ()
        {
            ...
            // calling on unowned pointer is not allowed
            Assert.That (() => list.First[1], Throws.InvalidOperationException);
        }
        ...
        public void TestNthPrev ()
        {
            ...
            Assert.That (nth, Is.Null);
            // check that unowned item is returned
            nth = list.Last.NthPrev (2);
            Assert.That (nth.Data.Value, Is.EqualTo (2));
            Assert.That (nth.Owned, Is.False);
        }
        ...
        {
            var itemInList = new TestWrappedNative (4);
            var itemNotInList = new TestWrappedNative (5);
            var list = new List<TestWrappedNative> ()
                .Append (new TestWrappedNative (1))
                .Append (new TestWrappedNative (2))
                .Append (new TestWrappedNative (3))
            ...
            // calling on unowned pointer is not allowed
            Assert.That (() => list.First.Find (new TestWrappedNative (1)),
                Throws.InvalidOperationException);
        }
        ...
        public void TestFindCustom ()
        {
            ...
            var list = new List<TestWrappedNative> ()
                ...
                .Append (new TestWrappedNative (4));
            // check that finding item returns unowned item
            var found = list.FindCustom (new TestWrappedNative (1), compareFunc);
            Assert.That (compareFuncWasCalled);
            Assert.That (found.Data.Value, Is.EqualTo (2));
            Assert.That (found.Owned, Is.False);
            // check that not finding item returns null
            found = list.FindCustom (new TestWrappedNative (4), compareFunc);
            Assert.That (found, Is.Null);
            // calling without compare func is not allowed
            Assert.That (() => list.FindCustom (new TestWrappedNative (1), null),
                Throws.InstanceOf<ArgumentNullException> ());
            // calling on unowned pointer is not allowed
            Assert.That (() => list.First.FindCustom (new TestWrappedNative (1), compareFunc),
                Throws.InvalidOperationException);
        }
        ...
        public void TestPosition ()
        {
            ...
            position = list.Position (new List<TestWrappedNative> ()
                .Append (new TestWrappedNative (1)));
        ...
        public void TestIndex ()
        {
            var list = new List<TestWrappedNative> ()
                ...
                .Append (new TestWrappedNative (4));
            // check that finding an item returns the index
            var index = list.IndexOf (new TestWrappedNative (3));
            Assert.That (index, Is.EqualTo (2));
            // check that not finding an item retures -1
            index = list.IndexOf (new TestWrappedNative (5));
