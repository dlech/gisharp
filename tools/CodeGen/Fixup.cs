    ...
    public static class Fixup
    {
        ...
        public static void Generate(this XDocument gir, TextWriter writer)
        {
            ...
            static string resolveNameAttribute(XElement element)
            {
                ...
                return isGI
                    ? $"@name='{value}'"
                    : $"@{resolveElementNamespace(element)}:name='{value}'";
            }

            var commands = gir.Element(gi + "repository")
                .Element(gi + "namespace")
                .Elements()
                ...
                .Select(
                    e =>
                        new SetAttribute
                        {
                            ...
                            Value = "0",
                            Xpath =
                                $"gi:repository/gi:namespace/{resolveElementNamespace(e)}:{e.Name.LocalName}[{resolveNameAttribute(e)}]"
                        }
                )
                .ToList<Command>();
            ...
            foreach (var command in commands)
            {
                switch (command)
                {
                    case AddElement addElement:
                        ...
                        {
                            logger.LogWarning(
                                "Could not find element at '{XPath}'",
                                addElement.Xpath
                            );
                            break;
                        }
                    ...
                    case SetAttribute setAttr:
                        ...
                        var setAttrElements = document
                            .XPathSelectElements(setAttr.Xpath, Manager)
                            .ToList();
                        if (setAttrElements.Count == 0)
                        {
                            logger.LogWarning(
                                "Could not find any elements matching '{XPath}'",
                                setAttr.Xpath
                            );
                            break;
                        }
                        foreach (var element in setAttrElements)
                        {
                            ...
                            else
                            {
                                element.SetAttributeValue(
                                    setAttrNamespace + setAttrLocalName,
                                    setAttr.Value
                                );
                            }
                        }
                    ...
                    case ChangeAttribute changeAttr:
                        ...
                        var changeAttrElements = document
                            .XPathSelectElements(changeAttr.Xpath, Manager)
                            .ToList();
                        ...
                        foreach (var element in changeAttrElements)
                        {
                            var attribute = element.Attribute(
                                changeAttrNamespace + changeAttrLocalName
                            );
                            var oldValue = attribute is null ? string.Empty : attribute.Value;
                            var newValue = string.IsNullOrEmpty(changeAttr.Regex)
                                ? changeAttr.Replace
                    ...
                    case ChangeElement changeElement:
                        ...
                        var elementsToChange = document.XPathSelectElements(
                            changeElement.Xpath,
                            Manager
                        );
                        ...
                        {
                            logger.LogWarning(
                                "Could not find elements matching '{XPath}'",
                                changeElement.Xpath
                            );
                            break;
                        }
                    ...
                    case MoveElement moveElement:
                        var moveElements = document
                            .XPathSelectElements(moveElement.Xpath, Manager)
                            .ToList();
                        ...
                        var moveParent = document.XPathSelectElement(
                            moveElement.NewParentXpath,
                            Manager
                        );
                    ...
                    case RemoveElement removeElement:
                        var removeElements = document
                            .XPathSelectElements(removeElement.Xpath, Manager)
                            .ToList();
        ...
        static bool IsSkipped(this XElement element)
        {
            ...
            bool IsAutoptrAlias()
            {
                ...
                return element
                    .Attribute("name")
                    .AsString("")
                    .EndsWith("_autoptr", StringComparison.Ordinal);
            }
        }
        ...
        public static void ApplyBuiltinFixup(this XDocument document)
        {
            ...
            var sharedLibrary = document
                .Element(gi + "repository")
                .Element(gi + "namespace")
            ...
            foreach (
                var element in document
                    .Descendants()
                    .Where(x => x.Elements(gi + "field").Any())
                    .ToList()
            )
            {
                ...
                )
                {
                    ...
                    if (combinedElement is null)
                    {
                        ...
                        size =
                            typeElement.Attribute("name").Value == "guint"
                                ? 32
            ...
            foreach (
                var element in document
                    ...
                    .Where(
                        x =>
                            x.Element(gi + "array") is XElement a
                            && a.Attribute("fixed-size").AsInt() > 0
                            && !a.Element(gi + "type")
                                .Attribute(c + "type")
                                .AsString()
                                .IsAllowedInFixedSizeBuffers()
                    )
                    .ToList()
            )
            {
                ...
                IEnumerable<XElement> generate()
                {
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        newElement
                            .Attribute("name")
                            .SetValue(newElement.Attribute("name").Value + i);
            ...
            // to the *Error enum.
            var errorQuarkFunctions = document
                .Descendants(gi + "function")
            ...
            foreach (var element in errorQuarkFunctions)
            {
                ...
                var errorDomainElement = document
                    .Descendants(gi + "enumeration")
            ...
            var elementsWithGTypeGetter = document
                .Descendants()
            ...
            var elementsThatThrow = document
                .Descendants()
            ...
            foreach (var element in document.Descendants(glib + "signal"))
            {
                ...
                parametersElement.AddFirst(
                    new XElement(
                        gi + "instance-parameter",
                ...
                parametersElement.Add(
                    new XElement(
                        ...
                        new XAttribute(
                            "closure",
                            parametersElement.Elements(gi + "parameter").Count()
                        ),
                        new XElement(
                            gi + "type",
                            new XAttribute("name", "gpointer"),
            ...
            foreach (
                var element in document
                    ...
                    .Where(
                        x =>
                            x.Element(gi + "parameters") is not null
                            || x.Element(gi + "return-value") is not null
                    )
            )
            {
            ...
            foreach (var element in document.Descendants())
            {
                ...
                if (
                    ...
                    || element.Name == gi + "virtual-method"
                )
                {
                    var returnTypeElement = element
                        .Element(gi + "return-value")
                        ?.Element(gi + "type");
                    if (
                        returnTypeElement?.Attribute("name")?.Value == "gboolean"
                        && !element.Attribute("throws").AsBool()
                    )
                    {
                        var paramElements = element
                            .Element(gi + "parameters")
                            .Elements(gi + "parameter");
                        if (
                            paramElements.Any(x => x.Attribute("direction").AsString("in") == "out")
                        )
                        {
                            name = "try_" + name;
                        }
                ...
                if (
                    accessModifier is not null
                    && (element.Name == gi + "field" || element.Name == gi + "constant")
                )
                {
                    camelCase = accessModifier.Value.Contains("private");
                }
                if (
                    element.Name == gi + "parameter"
                    || element.Name == gi + "instance-parameter"
                    || element.Name == gs + "error-parameter"
                )
                {
                    camelCase = true;
                }
            ...
            foreach (
                var element in document
                    .Descendants(gi + "constructor")
                    .Where(
                        x =>
                            x.Attribute("name")?.Value is string name
            ...
            )
            {
                ...
                var callbackElement = element
                    ...
                    .SingleOrDefault(
                        x =>
                            x.Element(gi + "type")?.Attribute(c + "type").AsString()
                            == "GAsyncReadyCallback"
                    );
                ...
                var finishMethodName = element
                    .Attribute(gs + "async-finish")
                    .AsString(asyncMethodName.Replace("_async", "_finish"));
                var finishMethodElement = element.Parent
                ...
                element.SetAttributeValue(
                    gs + "async-finish",
                    finishMethodElement.Attribute(gs + "managed-name").Value
                );
            }

            // flag ref methods

            var elementsWithRefMethod = document
                .Descendants(gi + "method")
                .Where(
                    d =>
                        d.Attribute("name").Value == "ref"
            ...
            var elementsWithUnrefMethod = document
                .Descendants(gi + "method")
                .Where(
                    d =>
                        d.Attribute("name").Value == "unref"
                        && !d.Element(gi + "parameters").Elements(gi + "parameter").Any()
                );
            ...
            var elementsWithCopyMethod = document
                .Descendants(gi + "method")
                .Where(
                    d =>
                        d.Attribute("name").Value == "copy"
                        && !d.Element(gi + "parameters").Elements(gi + "parameter").Any()
            ...
            var elementsWithFreeMethod = document
                .Descendants(gi + "method")
                .Where(
                    d =>
                        d.Attribute("name").Value == "free"
                        && !d.Element(gi + "parameters").Elements(gi + "parameter").Any()
                        && !d.Ancestors(gi + "class").Any()
                );
            ...
            var elementsWithCompareFunction = document
                .Descendants(gi + "method")
                .Where(
                    d =>
                        d.Attribute("name").Value == "compare"
            ...
            foreach (var element in elementsWithCompareFunction)
            {
                ...
                // convert to function
                element.Name = gi + "function";
                element.Element(gi + "parameters").Element(gi + "instance-parameter").Name =
                    gi + "parameter";
            }
            ...
            foreach (
                var element in document
                    ...
                    .Where(
                        d =>
                            d.Attribute("name").Value == "hash"
                            && !d.Element(gi + "parameters").Elements(gi + "parameter").Any()
                            && d.Element(gi + "return-value")
                                ?.Element(gi + "type")
                                ?.Attribute("name")
                                ?.Value == "guint"
                    )
            ...
            var elementsWithToStringFunction = document
                .Descendants(gi + "method")
                .Where(
                    d =>
                        d.Attribute("name").Value == "to_string"
                        && !d.Element(gi + "parameters").Elements(gi + "parameter").Any()
                );
            ...
            foreach (var element in typeElements)
            {
                ...
                if (cType is not null)
                {
                    ...
                    if (
                        cType.StartsWith("gpointer", StringComparison.Ordinal)
                ...
                if (
                    element.Parent.Name == gi + "parameter"
                    && element.Parent.Attribute("direction").AsString("in") != "in"
                )
                {
                    pointerDepth--;
                }
            ...
            foreach (
                var element in document
                    ...
                    .Where(
                        x =>
                            x.Element(gi + "type")?.Attribute("name").AsString() == "gboolean"
            ...
            foreach (
                var element in document
                    ...
                    .Where(
                        x =>
                            x.Element(gi + "type")?.Attribute(c + "type")?.Value == "GCancellable*"
            ...
            foreach (
                var element in document
                    ...
                    .Where(
                        x =>
                            (
                                x.Parent.Parent.Name == gi + "method"
                                || x.Parent.Parent.Name == gi + "function"
                            )
            ...
            var getters = document
                .Descendants()
                .Where(
                    d =>
                        (d.Name == gi + "function" || d.Name == gi + "method")
                        && !d.Attribute(gs + "extension-method").AsBool()
                        && !d.Attribute(gs + "pinvoke-only").AsBool()
                        && (
                            d.Attribute("name").Value.StartsWith("get_", StringComparison.Ordinal)
                            || d.Attribute("name").Value.StartsWith("is_", StringComparison.Ordinal)
                        )
                        && (
                            d.Element(gs + "managed-parameters") is null
                            || !d.Element(gs + "managed-parameters")
                                .Elements(gi + "parameter")
                                .Any()
                        )
                );
            foreach (var element in getters)
            {
                ...
                var propertyElement = new XElement(element) { Name = gs + "managed-property" };
            ...
            var setters = document
                ...
                .Where(
                    d =>
                        ...
                        && d.Element(gs + "managed-parameters").Elements(gi + "parameter").Count()
                            == 1
                );
            foreach (var element in setters)
            {
                ...
                // drop the Set prefix
                name = name[3..];
                var matchingGetter = element.Parent
                    .Elements(element.Name)
                    .SingleOrDefault(
                        e => e.Attribute(gs + "property-getter-for").AsString() == name
                    );
            ...
            foreach (var element in document.Descendants(gi + "property"))
            {
                ...
                var managedPropertyElement = element.Parent
                    .Elements(gs + "managed-property")
        ...
        {
            foreach (
                var e in doc.Descendants().Where(d => ElementsThatDefineAType.Contains(d.Name))
            )
            {
            ...
            foreach (
                var e in doc.Descendants(gi + "namespace")
                    .Elements(gi + "constant")
                    .Where(x => !x.IsSkipped())
            )
            {
        ...
        static IEnumerable<XElement> EnumerateManagedParameters(this XElement parameters)
        {
            ...
            indexesToRemove.AddRange(
                list.Where(p => p.GetLengthIndex() >= 0).Select(p => p.GetLengthIndex())
            );
            indexesToRemove.AddRange(
                list.Where(
                        p =>
                            p.Element(gi + "type")?.Attribute(c + "type")?.Value
                            == "GAsyncReadyCallback"
                    )
            ...
            list = list.Where(
                    p => p.Attribute(gs + "default") is null && p.Attribute(gs + "params") is null
                )
                .Union(
                    list.Where(
                        p =>
                            p.Attribute(gs + "default") is not null
                            && p.Attribute(gs + "params") is null
                    )
                )
                .Union(list.Where(p => p.Attribute(gs + "params") is not null))
                .ToList();
            ...
            if (
                parameters.Parent.Attribute(gs + "extension-method").AsBool()
                || parameters.Parent.Name == glib + "signal"
            )
            {
        ...
        public static string ToCamelCase(this string str)
        {
            ...
            str = ((str.Length > 0) ? str[..1].ToLower() : "") + ((str.Length > 1) ? str[1..] : "");
