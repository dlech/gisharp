...
namespace GISharp.CodeGen.Syntax
{
    public static class GIArgExtensions
    {
        ...
        public static string GetSpecializedManagedType(this GIArg arg)
        {
            ...
            if (
                arg is ReturnValue returnValue
                && returnValue.Callable is Method method
                && method.IsHash
            )
            {
            ...
            if (
                arg.Type.TypeParameters.Any()
                && arg.Type.GirName != "GLib.ByteArray"
                && arg.Type.GirName != "bytestring"
            )
            {
                ...
                if (type == "GISharp.Runtime.CArray")
                {
                    ...
                    if (isAsync)
                    {
                        if (isZeroTerminated)
                        {
                            throw new NotSupportedException(
                                "don't know how to handle zero-terminated CArray in async"
                            );
                        }
                        if (arg.Direction == "out" && arg.IsCallerAllocates)
                        {
                    ...
                    else if (arg.Direction == "out" && arg.IsCallerAllocates)
                    {
                        ...
                        {
                            throw new NotSupportedException(
                                "don't know how to handle caller allocated zero-terminated CArray"
                            );
                        }
                        type = "System.Span";
                    }
                    ...
                    else if (isZeroTerminated)
                    {
                        if (arg.TransferOwnership == "none")
                        {
                            if (arg.IsNullable)
                            {
                                type = type.Replace(
                                    "CArray",
                                    "NullableUnownedZeroTerminatedCArray"
                                );
                            }
                            else
                            {
                ...
                else
                {
                    if (isZeroTerminated)
                    {
                        ...
                        {
                            throw new NotSupportedException(
                                $"don't know how to handle zero-terminated {type}"
                            );
                        }
                    }
                    ...
                    if (arg.Direction == "out" && arg.IsCallerAllocates)
                    {
                        throw new NotImplementedException(
                            "Not sure how to handle caller allocated pointer array"
                        );
                    }
                ...
                var typeParameters = arg.Type.TypeParameters.Select(
                    x => useUnmanagedElementType ? x.GetUnmanagedType() : x.GetManagedType()
                );
        ...
        public static ParameterSyntax GetParameter(this GIArg arg, string suffix = "")
        {
            ...
            if (
                ...
                && !type.Contains('*')
                && type != "System.IntPtr"
                && !type.Contains("Unowned", StringComparison.Ordinal)
        ...
        )
        {
            ...
            if (arg.ParentNode is Parameters)
            {
                if (
                    ...
                    && !(arg.Direction == "inout" && arg.Type.IsValueType())
                )
                {
        ...
        public static StatementSyntax[] GetMarshalManagedToUnmanagedStatements(
            this GIArg arg,
            out FixedStatementSyntax fixedStatement,
            bool declareVariable = true
        )
        {
            fixedStatement = null;

            var @var = declareVariable
                ? "var "
                : (arg.Direction != "in" && !arg.IsCallerAllocates ? "*" : "");
            ...
            if (
                arg.Type is Gir.Array array
                && array.GirName is null
                && !type.Contains("GISharp.Lib.GLib.Strv")
            )
            {
                        ...
                        VariableDeclaration(dataType)
                            .AddVariables(
                                VariableDeclarator($"{arg.ManagedName}Data_")
                                    .WithInitializer(
                                        EqualsValueClause(ParseExpression(arg.ManagedName))
                                    )
                            ),
                        Block()
                    );
                ...
                {
                    var lengthArg = arg.Callable.Parameters.RegularParameters.ElementAt(
                        array.LengthIndex
                    );
                    var lengthIdentifier = lengthArg.ManagedName;
                    var lengthType = lengthArg.Type.GetUnmanagedType();
                    var lengthGetter =
                        (arg.IsNullable && !isSpanLike)
                            ? $"{arg.ManagedName}?.Length ?? 0"
                            : $"{arg.ManagedName}.Length";
                    if (takeData)
                    {
                    ...
                    // matches the first
                    if (
                        arg.Callable.ManagedParameters
                            .TakeWhile(x => x != arg)
                    ...
                    )
                    {
                        ...
                        var exception =
                            $"System.ArgumentException(\"Length length of {arg.ManagedName} must be the same as {first.ManagedName}\", nameof({arg.ManagedName}))";
                        expressions.Add(
                            ParseExpression(
            ...
            else if (arg.Type.IsOpaque())
            {
                if (arg.Type.GirName == "utf8" && arg.TransferOwnership == "none")
                {
                    expressions.Add(
                        ParseExpression(
                            $"{@var}{unmanagedName} = {unmanagedCast}{arg.ManagedName}.UnsafeHandle"
                        )
                    );
                }
                else
                {
                    ...
                    expressions.Add(
                        arg.IsNullable
                            ? ParseExpression(
                                $"{@var}{unmanagedName} = {unmanagedCast}({arg.ManagedName}?.{getter} ?? System.IntPtr.Zero)"
                            )
                            : ParseExpression(
                                $"{@var}{unmanagedName} = {unmanagedCast}{arg.ManagedName}.{getter}"
                            )
                    );
                }
            }
            else if (arg.Type.IsValueType())
            {
                ...
                if ((arg.Direction == "out" && arg.IsCallerAllocates) || arg.Direction == "inout")
                {
                    fixedStatement = FixedStatement(
                        VariableDeclaration(ParseTypeName($"{unmanagedType}*"))
                            .AddVariables(
                                VariableDeclarator(unmanagedName)
                                    .WithInitializer(
                                        EqualsValueClause(ParseExpression($"&{arg.ManagedName}"))
                                    )
                            ),
                ...
                else if (arg.Type.GirName == "gboolean")
                {
                    expressions.Add(
                        ParseExpression(
                            $"{@var}{unmanagedName} = {typeof(BooleanExtensions)}.{nameof(BooleanExtensions.ToBoolean)}({arg.ManagedName})"
                        )
                    );
                }
                else if (arg.Type.GirName == "gunichar")
                {
                    ...
                    expressions.Add(
                        ParseExpression(
                            $"{@var}{unmanagedName} = {unmanagedCast}{arg.ManagedName}.Value"
                        )
                    );
                }
                else
                {
                    expressions.Add(
                        ParseExpression($"{@var}{unmanagedName} = {unmanagedCast}{arg.ManagedName}")
                    );
                }
            }
            ...
            {
                if (
                    arg.Callable.Parameters.RegularParameters.ElementAtOrDefault(arg.ClosureIndex)
                    is Parameter userData
                )
                {
                ...
                if (
                    arg.Callable.Parameters.RegularParameters.ElementAtOrDefault(arg.DestroyIndex)
                    is Parameter destroy
                )
                {
                    var nullCheck = arg.IsNullable ? $"{arg.ManagedName} is null ? default : " : "";
                    expressions.Add(
            ...
            else
            {
                expressions.Add(
                    ParseExpression(
                        $"throw new System.NotImplementedException(\"{nameof(GetMarshalManagedToUnmanagedStatements)}\")"
                    )
                );
            }
        ...
        public static StatementSyntax[] GetMarshalUnmanagedToManagedStatements(
            this GIArg arg,
            bool declareVariable = true
        )
        {
            ...
            if (type.Contains("GISharp.Lib.GLib.Strv"))
            {
                ...
                if (arg.Type is Gir.Array v && v.LengthIndex >= 0)
                {
                    ...
                    var lengthParameter = arg.Callable.Parameters.RegularParameters.ElementAt(
                        v.LengthIndex
                    );
                ...
                expressions.Add(
                    ParseExpression(
                        $"{@var}{arg.ManagedName} = new {type}((System.IntPtr){arg.ManagedName}_, {lengthArg}, {ownership})"
                    )
                );
            }
            else if (arg.Type is Gir.Array array && array.GirName is null)
            {
                ...
                var getter =
                    $"new {type}((System.IntPtr){arg.ManagedName}_, {lengthArg}, {ownership})";

                if (
                    type.Contains("Unowned", StringComparison.Ordinal)
                    || type.Contains("Span", StringComparison.Ordinal)
                )
                {
            ...
            {
                expressions.Add(
                    ParseExpression($"{@var}{arg.ManagedName} = new {type}({arg.ManagedName}_)")
                );
            }
            else if (arg.Type.IsValueType())
            {
                // value types are used directly
                if (
                    (arg is ReturnValue returnValue && returnValue.IsRefReturn())
                    || (arg.Direction == "in" && arg.IsByRefValueType())
                )
                {
                    // TODO: consider "readonly" here if arg type is const
                    expressions.Add(
                        ParseExpression(
                            $"ref {@var}{arg.ManagedName} = ref System.Runtime.CompilerServices.Unsafe.AsRef<{type}>({arg.ManagedName}_)"
                        )
                    );
                }
                else if (arg.Type.GirName == "gboolean")
                {
                    expressions.Add(
                        ParseExpression(
                            $"{@var}{arg.ManagedName} = {typeof(BooleanExtensions)}.{nameof(BooleanExtensions.IsTrue)}({arg.ManagedName}_)"
                        )
                    );
                }
                else
                {
                    expressions.Add(
                        ParseExpression($"{@var}{arg.ManagedName} = ({type}){arg.ManagedName}_")
                    );
                }
            }
            else if (arg.Type.Interface is Interface)
            {
                ...
                expressions.Add(
                    ParseExpression(
                        $"{@var}{arg.ManagedName} = ({type}{nullable}){getInstance}((System.IntPtr){arg.ManagedName}_, {ownership}){notNullable}"
                    )
                );
            }
            ...
            else if (arg.Type.IsOpaque())
            {
                ...
                expressions.Add(
                    ParseExpression(
                        $"{@var}{arg.ManagedName} = {type}.GetInstance<{type}>((System.IntPtr){arg.ManagedName}_, {ownership}){notNullable}"
                    )
                );
            }
            else
