            ...
            if (extraFixups)
            {
                ...
                foreach (var prefix in ns.CIdentifierPrefixes)
                {
                    ...
                    foreach (Match c in constants.OrderByDescending(x => x.Value.Length))
                    {
                        ...
                        builder.Replace(
                            c.Value,
                            $"<see cref=\"{parent.ManagedName}.{member.ManagedName}\"/>"
                        );
                    }
                    ...
                    {
                        var type = ns.AllTypes.SingleOrDefault(
                            x =>
                                ((x as GIRegisteredType)?.CType ?? (x as Callback)?.CType)
                                == t.Value[1..]
                        );
                        if (type is null)
                        {
                ...
                foreach (Match p in paramRefs.OrderByDescending(x => x.Value.Length))
                {
                    ...
                    if (callable is not null)
                    {
                        ...
                        if (
                            instanceParam?.GirName == p.Value[1..]
                            && !(callable is Method m && m.IsExtensionMethod)
                        )
                        {
                        ...
                        foreach (
                            var (arrayArg, type) in callable.Parameters
                                .Select(x => (x, x.Type as Array))
                                .Where(x => x.Item2?.LengthIndex >= 0)
                        )
                        {
                            if (
                                callable.Parameters.RegularParameters
                                    .ElementAt(type.LengthIndex)
                                    .GirName == p.Value[1..]
                            )
                            {
                                builder.Replace(
                                    p.Value,
                                    $"the length of <paramref name=\"{arrayArg.ManagedName}\"/>"
                                );
                                continue;
                            }
                        }
                        ...
                        if (
                            callable.Parameters.SingleOrDefault(x => x.GirName == p.Value[1..])
                            is GIArg arg
                        )
                        {
                            builder.Replace(
                                p.Value,
                                $"<paramref name=\"{arg.ManagedName.Replace("@", "")}\"/>"
                            );
                            continue;
                        }
                    }
                    ...
                    {
                        if (
                            declaringType.Fields.SingleOrDefault(x => x.GirName == p.Value[1..])
                            is Field field
                        )
                        {
                        ...
                        if (
                            declaringType.GTypeStruct is not null
                            && declaringType.ParentNode is Namespace @namespace
                        )
                        {
                            var gtype =
                                @namespace.Records.Single(
                                    x =>
                                        x is GIRegisteredType t
                                        && t.GirName == declaringType.GTypeStruct
                                ) as GIRegisteredType;
                            if (
                                gtype.Fields.SingleOrDefault(x => x.GirName == p.Value[1..])
                                is Field gtypeField
                            )
                            {
                                builder.Replace(
                                    p.Value,
                                    $"<see cref=\"Do{gtypeField.ManagedName}\"/>"
                                );
                                continue;
                            }
                        }
                    ...
                    foreach (var f in functions.Cast<Match>())
                    {
                        ...
                        if (callable is Method m && m.IsExtensionMethod)
                        {
                            parameters = parameters.Prepend(
                                callable.ManagedParameters.ThisParameter
                            );
                        }
                        method +=
                            $"({string.Join(",", parameters.Select(x => getManagedType(x)))})";
                        builder.Replace(
                            $"{f.Value}()",
                            $"<see cref=\"{parent.GirName}.{method}\"/>"
                        );
                    }
                }
            }
