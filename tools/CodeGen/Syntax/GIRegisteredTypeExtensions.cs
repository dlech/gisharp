...
namespace GISharp.CodeGen.Syntax
{
    public static class GIRegisteredTypeExtensions
    {
        ...
        public static SyntaxList<AttributeListSyntax> GetGTypeAttributeLists(
            this GIRegisteredType type
        )
        {
            ...
            if (type.GTypeName is not null)
            {
                ...
                list = list.Add(
                    AttributeList()
                        .AddAttributes(
                            Attribute(attrName).AddArgumentListArguments(nameArg, isProxyArg)
                        )
                );
            }
            ...
            if (type.GTypeStruct is not null)
            {
                ...
                list = list.Add(
                    AttributeList()
                        .AddAttributes(Attribute(attrName).AddArgumentListArguments(typeArg))
                );
            }
        ...
        public static ConstructorDeclarationSyntax GetDefaultConstructor(this GIRegisteredType type)
        {
            var parameterList = ParseParameterList(
                string.Format("({0} handle, {1} ownership)", typeof(IntPtr), typeof(Transfer))
            );
            ...
            var accessModifier =
                (type is Class @class && @class.IsAbstract)
            ...
            if (type.Methods.SingleOrDefault(x => x.IsRef || x.IsCopy) is Method method)
            {
                ...
                body = body.AddStatements(
                    IfStatement(
                        ParseExpression($"ownership == {typeof(Transfer)}.{nameof(Transfer.None)}"),
                        Block(
                            ExpressionStatement(
                                ParseExpression(
